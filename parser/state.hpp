...
typedef std::set<std::unique_ptr<State>, Same_State> Unique_States;

/*******************************************************************************
 * Set of unique actions for the individual states to reference.
 */
struct Same_Actions {
    bool operator()(const std::unique_ptr<State::Actions>& a,
                    const std::unique_ptr<State::Actions>& b) const {
        return *a < *b;
    }
};

typedef std::set<std::unique_ptr<State::Actions>, Same_Actions> Unique_Actions;
